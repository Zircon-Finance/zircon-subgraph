...
import { PylonUpdate } from '../types/templates/Pylon/Pylon'
...
export function handlePylonSync(event: PylonUpdate): void {
  ...
  pylonFactory.totalLiquidityETH = pylonFactory.totalLiquidityETH.minus(pylon.trackedReserveETH as BigDecimal)

  // reset token total liquidity amounts
  token0.totalLiquidity = token0.totalLiquidity.minus(pylon.reserve0)
  token1.totalLiquidity = token1.totalLiquidity.minus(pylon.reserve1)
  ...
  pylon.reserveETH = pylon.reserve0
    .times(token0.derivedETH as BigDecimal)
  ...
  token1.totalLiquidity = token1.totalLiquidity.plus(pylon.reserve1)
