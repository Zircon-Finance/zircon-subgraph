...
export class PylonFactory extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save PylonFactory entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save PylonFactory entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("PylonFactory", id.toString(), this);
  }

  static load(id: string): PylonFactory | null {
    return store.get("PylonFactory", id) as PylonFactory | null;
  }
